...
std::string restore_from_prefix_func(const std::vector<size_t>& pref_func) {
    std::string answer;
    ...
    for (size_t i = 1; i < pref_func.size(); ++i) {
        ...
        size_t left_letter = least_significant_bit(alphabet);
...
int main() {
    ...
    std::string answer = restore_from_prefix_func(input);
    std::cout << answer;
    return 0;
}
